...
export const onRequest = async ({ env, request }) => {
  ...
  async function loadArray(key){ try{ return JSON.parse(await KV.get(key) || '[]'); }catch{ return []; } }
  ...
  async function appendHistory(line){
    ...
    arr.unshift(entry); await saveArray('history', arr);
  }
  ...
  if (m) {
    ...
    if (action==='allocate' && method==='POST') {
      ...
      order.status='allocated'; await KV.put(`order:${orderId}`, JSON.stringify(order));
