...
export const onRequest = async ({ env, request }) => {
  ...
  const mDel = url.pathname.match(/^\/api\/orders\/(\d+)$/);
  if (mDel && method === "DELETE") {
    const id = mDel[1];
    const exist = await env.ESTOQUE_DB.get(keyOrder(id), "json");
    if (!exist) return json({ ok:false, error:"not found" }, 404);
    await env.ESTOQUE_DB.delete(keyOrder(id));
    const ids = await listIds(env);
    await saveIds(env, ids.filter(x => x !== id));
    await addHistory(env, "PEDIDO_EXCLUIDO", `Pedido ${exist.referencia} (${id}) removido`);
    return json({ ok:true });
  }
