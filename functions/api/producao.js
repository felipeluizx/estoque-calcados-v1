...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const items = (results || []).map(item => {
      ...
      return { ...item, quantity_remaining: remaining, production_status: status, line_total: item.unit_price == null ? null : Number(item.unit_price) * ordered };
    });
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const item = await env.DB.prepare(`
      SELECT oi.id, oi.quantity_ordered, oi.quantity_cancelled,
    ...
    `).bind(orderItemId, quantity, body.notes ? String(body.notes).trim() : null).first();

    const newRemaining = remaining - quantity;
    return json({ ok: true, movement, quantity_remaining: newRemaining, production_status: newRemaining === 0 ? "completed" : "partial" }, 201);
  } catch (err) {
    return json({ ok: false, error: err.message }, 500);
  }
}
