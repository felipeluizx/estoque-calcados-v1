...
(function(){
  function ensurePages(){
    ...
    const mk = (id) => {
      if (!document.getElementById(id)) {
        ...
        d.id = id;
        d.className = 'page hidden';
        main.appendChild(d);
      }
    };
    mk('page-pedidos'); mk('page-organizacao');
  }
  ...
  function addDesktopNav(){
    ...
    if (!nav) return;
    const make = (label, hash) => {
      if (nav.querySelector(`a[href="${hash}"]`)) return;
      const a = document.createElement('a');
      a.href = hash;
      a.textContent = label;
      a.className = 'menu-item flex items-center px-4 py-2 rounded-md hover:bg-gray-700';
      nav.appendChild(a);
    };
    make('Pedidos', '#pedidos');
    make('Organização', '#organizacao');
  }

  function addMobileNav(){
    // procura nav móvel (tem grid e menu-item)
    const navs = Array.from(document.querySelectorAll('nav')).filter(n => n.querySelector('a.menu-item'));
    const mobile = navs.find(n => getComputedStyle(n).position === 'sticky' || n.className.includes('md:hidden'));
    if (!mobile) return;
    const make = (label, hash) => {
      if (mobile.querySelector(`a[href="${hash}"]`)) return;
      const a = document.createElement('a');
      a.href = hash;
      a.className = 'menu-item flex flex-col items-center text-center p-2 rounded-lg';
  ...
  function integrateWithRouter(){
    // o index já tem navigate()/rerender que mostram .page pelo hash
    // então só precisamos garantir que nossas páginas existam e, se abertas,
    // renderizá-las.
    function renderPedidos(){
      ...
      document.getElementById('od_create').onclick = async () => {
        ...
        if (!j.ok) { document.getElementById('od_msg').textContent = j.error||'Erro ao criar'; return; }
        orderId = j.order.id;
      ...
      function chip(container, values, onChange){
      ...
      chip(document.getElementById('f_modelo'), modelos, v=>sel.modelo=v);
      chip(document.getElementById('f_grade'),  grades,  v=>sel.grade=v);
      chip(document.getElementById('f_material'), mats, v=>sel.material=v);
      chip(document.getElementById('f_variacao'), vars, v=>sel.variacao=v);
    ...
    // Integração com o router existente
    const handler = () => {
      const h = location.hash || '#home';
      if (h === '#pedidos') renderPedidos();
      if (h === '#organizacao') renderOrganizacao();
    };
